...
public class SQLiteManager extends SQLiteOpenHelper
{
    ...
    private static final String TABLE_NAME = "Notess";
...
//        }
    ...
    public void addNoteToDatabase(Note note)
    {
        ...
        contentValues.put(ARRAY_FIELD, note.getArray());
        contentValues.put(SIZE_FIELD, note.getSize());
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
        {
            if(result.getCount() != 0)
            {
                while (result.moveToNext())
                {
                    ...
                    String array = result.getString(3);
                    int size = result.getInt(4);
                    String stringDeleted = result.getString(5);
                    Date deleted = getDateFromString(stringDeleted);
                    Note note = new Note(id,title,array,size, deleted);
